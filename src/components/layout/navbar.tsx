        ...
        <div className="mx-auto flex h-16 max-w-6xl items-center justify-between px-4 md:px-6">
          ...
          <div className="hidden md:flex items-center gap-4">
            <div className="group hidden lg:flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-3 py-1.5 text-xs text-muted-foreground transition-colors hover:bg-white/10 hover:text-foreground cursor-text">
              ...
              <kbd className="ml-2 rounded-md bg-white/10 px-1.5 py-0.5 font-sans text-[10px]">⌘K</kbd>
            </div>
            <div className="w-[1px] h-4 bg-white/10 mx-1 hidden lg:block" />
            <Link href="/sign-in" className="text-sm font-medium text-muted-foreground hover:text-foreground transition-colors">
              Log in
            </Link>
            <Link href="/sign-up" className="rounded-full bg-foreground px-5 py-2 text-sm font-semibold text-background transition-transform hover:scale-105 active:scale-95 shadow-lg">
              Start Building
            </Link>
          </div>
          ...
          >
            <div className="flex flex-col h-full p-4">
              ...
              <nav className="flex flex-col gap-6 text-2xl font-semibold mb-12">
                ...
                <Link href="#components" onClick={() => setMobileMenuOpen(false)}>Components</Link>
                <Link href="#pricing" onClick={() => setMobileMenuOpen(false)}>Pricing</Link>
                <Link href="#docs" onClick={() => setMobileMenuOpen(false)}>Documentation</Link>
              </nav>
              <div className="mt-auto flex flex-col gap-4">
                <Link href="/sign-in" className="w-full rounded-xl border border-white/10 px-4 py-4 text-center text-lg font-medium transition-colors hover:bg-white/5" onClick={() => setMobileMenuOpen(false)}>
                  Log in
                </Link>
                <Link href="/sign-up" className="w-full rounded-xl bg-foreground px-4 py-4 text-center text-lg font-semibold text-background shadow-lg" onClick={() => setMobileMenuOpen(false)}>
                  Start Building
                </Link>
              </div>
            </div>
